...
application to send 30 PLI reports and 10 images per minute.

```bash
> stack exec resource-dsl -- example network --dict --model --reqs --init 1000 --config \(30,10\)
```
...
the bandwidth is insufficient for the configured PLI and image report rate.

The following command will reconfigure to only send 5 images per minute:

```bash
> stack exec resource-dsl -- example network --config \(30,5\)
```
