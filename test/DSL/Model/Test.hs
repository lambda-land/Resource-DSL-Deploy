...
checkUnit :: Name -> Stmt
checkUnit n = Do (Path Absolute [n]) (Check (Fun (Param "x" TUnit) true))

provideUnit :: Name -> Stmt
provideUnit n = Do (Path Absolute [n]) (Create (Lit Unit))

-- | blockone tests Do stmts and an In stmt
blockone :: Block
blockone = [provideUnit "x", checkUnit "x", In (Path Absolute ["foo"]) [checkUnit "x"]]
...
ctx' = Ctx (ResID ["foo"])
  ...
  (envSingle "subprofile" (ProEntry (profile [Param "p" TInt ,Param "s" TBool]
                                     [(Path Absolute ["foo"], [Create (Ref "xx")])])))
--
-- ** Tests
...
testModel = testGroup "Model Tests"
  [ testGroup "test toProfile"
    ...
    [ testCase "toProfile converts a Model to a Profile, with checkUnit" $
      do out <- return $ toProfile (Model [Param "x" TInt] [checkUnit "x"])
         p <- return $ Profile [Param "x" TInt]
           (envFromList [(Path Absolute ["x"], [checkFor "x"])])
         p @=? out

    , testCase "toProfile converts a Model to a Profile, with a Block" $
      do out <- return $ toProfile (Model [Param "x" TInt] [provideUnit "x"
                                                       , checkUnit "x"])
         p <- return $ Profile [Param "x" TInt]
           (envFromList [(Path Absolute ["x"], [create, checkFor "x"])])
         p @=? out

    , testCase "toProfile creates a Profile wih an In block" $
      do out <- return $ toProfile (Model [Param "x" TInt] blockone)
         p <- return $ Profile [Param "x" TInt]
           (envFromList [(Path Absolute ["foo", "x"], [checkFor "x"])
                        ,(Path Absolute ["x"], [create , checkFor "x"])])
         p @=? out
  ...
  , testGroup "test profileDict"
    ...
    [ testCase "profileDict creates a Dict with single Effect" $
      do out <- return $ profileDict [("x", Model [Param "y" TInt]
                                       [provideUnit "x"])]
         p <- return $ envFromList [("x"
                                    , ProEntry
                                      (Profile [Param "y" TInt]
                                       (envSingle (Path Absolute ["x"])
                                        [(Create (Lit Unit))])))]
         p @=? out
    ]
  ...
  , testGroup "test composeModels"
    ...
    [ testCase "composeModels unions two Models with distinct Params" $
      do
        ...
        p <- return $ Model [Param "x" TInt, Param "y" TBool, Param "z" TUnit]
          (blockone ++ blockone)
        p @=? out
